...
def small_text(s, color="#888"):
    st.markdown(f'<span style="font-size:0.95em;color:{color}">{s}</span>', unsafe_allow_html=True)

if "token" not in st.session_state:
    st.session_state.token = None

# Minimal header, centered
st.markdown('<div style="text-align:center;"><h2 style="margin-bottom:2px;">Guided Weather</h2><span style="font-size:1.5em;">🌤️</span></div>', unsafe_allow_html=True)
...
if st.session_state.token is None:
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["Weather", "Your favorites", "Weather Summary", "Ask about Weather"])

    # ---- WEATHER TAB ----
    with tab1:
        small_text("Search for any city's weather")
        city_query = st.text_input("", placeholder="e.g. London or Paris", key="search", label_visibility="collapsed")
        ...
        for fav in favs:
            ...
            with col2:
                if st.button("✖", key=f"rem_{fav['city']}"):
                    ...
                    st.rerun()
        st.markdown("---")
        add_city = st.text_input("Add city to favorites", key="addfav", placeholder="e.g. Madrid")
    ...
    with tab3:
        ...
        if st.button("get summary", use_container_width=True):
            ...
            if resp.ok:
                st.markdown(f'<div style="font-size:1.08em;color:#aaa;">{resp.json()["summary"]}</div>', unsafe_allow_html=True)
            else:
    ...
    with tab4:
        ...
        ask = st.text_input("your weather question", key="weatherq", placeholder="e.g. which cities are sunny now?")
        if st.button("ask LLM", use_container_width=True):
            with st.spinner("the LLM is reasoning..."):
                resp = requests.post(f"{API_URL}/ask", headers=headers, json={"question": ask})
            if resp.ok:
                ans = resp.json()
                st.markdown(f"<b>LLM:</b> {ans.get('answer', '')}", unsafe_allow_html=True)
                if ans.get("matchingCities"):
                    st.markdown(
                        f"<b>matching cities:</b> {', '.join(ans['matchingCities'])}", unsafe_allow_html=True)
            else:
                st.error("could not get LLM answer")

    st.markdown("---")
    st.button("logout", use_container_width=True, on_click=lambda: (st.session_state.pop("token", None), st.rerun()))
